<?php
/**
 * original_theme functions and definitions
 *
...
function original_setup() {
	/*
		...
		* If you're building a theme based on original_theme, use a find and replace
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'original_custom_background_args',
			array(
...
function original_scripts() {
	wp_enqueue_style( 'original-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'original-style', 'rtl', 'replace' );

	wp_enqueue_script( 'original-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
